...
call_ru()
{
        ...
        oci db patch list by-database --database-id $DB_OCID --profile MY-PROFILE --all #--output table
        echo " "
        echo "### OCID for DB RU Patch"
        export PATCH_OCID=$(oci db patch list by-database --database-id $DB_OCID --profile MY-PROFILE --all --query "data[?contains(\"version\",'$VERSION_TO_APPLY')]" |  jq -r '.[]."id"');echo $PATCH_OCID
        echo " "
        echo "### DB RU Apply Action " $ACTION
        oci db database patch --database-id $DB_OCID --patch-id $PATCH_OCID --patch-action $ACTION  --profile MY-PROFILE
}
## Main
if [ -z "$5" ]
then
        ...
        echo " "        
        echo "E.g.  # ./ocidpatch_ru.sh DEV devdb01 orcl 19.XX.0.0.0 PRECHECK "
        echo "E.g.  # ./ocidpatch_ru.sh DEV devdb01 orcl 19.XX.0.0.0 APPLY "
        echo " "        
